...
//    }
    ...
    public ResponseEntity<?> add(@RequestBody DocumentRH documentRH,
        ...
        try {
            ...
            documentRH.setValidateur(utilisateurs_validateur);
            ...
            responseObject.put("message", "Document RH créé avec succès."); // Message de succès
            responseObject.put("data", savedDocumentRH); // Les informations du document RH créé
            return ResponseEntity.status(HttpStatus.CREATED).body(responseObject.toString());
        } catch (Exception e) {
            // En cas d'erreur lors de la création du document RH
            JSONObject errorResponse = new JSONObject();
            errorResponse.put("success", 0); // Échec
            errorResponse.put("message", "Erreur lors de la création du document RH."); // Message d'erreur
