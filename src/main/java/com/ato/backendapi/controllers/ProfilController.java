...
public class ProfilController {
    ...
    public ResponseEntity<?> addProfil(@RequestBody Profil profil) {
        try {
            Profil savedProfil = profilService.createProfilWithHabilitations(profil);

            // Créer l'objet JSON de retour pour succès
            JSONObject responseObject = new JSONObject();
            responseObject.put("success", 1); // Succès
            responseObject.put("message", "Profil créé avec succès."); // Message de succès
            ...
            errorResponse.put("message", "Erreur lors de la création du profil."); // Message d'erreur
