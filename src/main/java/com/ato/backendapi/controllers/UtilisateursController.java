...
//    }
    ...
    public ResponseEntity<UtilisateursDTO> getUserById(@PathVariable("id") Long id) {
        ...
        UtilisateursDTO utilisateurDTO = new UtilisateursDTO();
            ...
            if (departements != null) {
                utilisateurDTO.setDepartementId(departements.getIdDepartement());
                utilisateurDTO.setDepartementDescription(departements.getDescription());
                //utilisateurDTO.setDirectionId(departements.getDirection().getIdDirection());
                //utilisateurDTO.setDirectionDescription(departements.getDirection().getDescription());
            }
    ...
    //POST
    @PostMapping("/users")
    public ResponseEntity<?> add(@RequestBody Utilisateurs utilisateurs,
                                 @RequestParam(required = false) Long idDepartement,
        ...
        try {
            ...
            responseObject.put("message", "Utilisateur et compteur créés avec succès."); // Message de succès
            responseObject.put("data", savedUser); // Les informations de l'utilisateur créé
            ...
            errorResponse.put("message", "Erreur lors de la création de l'utilisateur."); // Message d'erreur
...
@PostMapping("/signIn")
public ResponseEntity<?> authenticateUserWithIdentifiantAndPassword(@RequestBody SignInRequest authenticationRequest) {
...
//    }
    ...
    //PUT
    @PutMapping("/users/{id}")
    public ResponseEntity<?> edit(@PathVariable("id") Long id, @RequestBody Utilisateurs utilisateurs,
                                  @RequestParam Long idDepartement, @RequestParam Long idContrats,
        ...
        // Vérifier si les IDs des départements, des contrats et des profils sont valides
        Departements departement = departementsRepository.findById(idDepartement)
        ...
        existingUser.setRole(utilisateurs.getRole());
        ...
        if (savedUser != null) {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Une erreur est survenue lors de la modification de l'utilisateur.");
        }
    }
